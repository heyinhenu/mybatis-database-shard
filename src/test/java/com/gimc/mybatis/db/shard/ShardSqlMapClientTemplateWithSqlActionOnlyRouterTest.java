...
import com.gimc.mybatis.db.shard.entities.Follower;
...
public class ShardSqlMapClientTemplateWithSqlActionOnlyRouterTest extends AbstractTestNGCobarClientTest {

    public ShardSqlMapClientTemplateWithSqlActionOnlyRouterTest() {
    ...
    /**
     * insert data onto default data source , and query will against all of the partitions, so all of the records will be returned as expected.
     */
    ...
    /**
     * although records are inserted onto patition2, but since the query is against all of the data sources, so all of the records will be returned as
     * expected.
     */
    ...
    /**
     * insert records onto partition1, but the 'com.alibaba.cobar.client.entities.Follower.finaByName' will be performed against partition2 as per the
     * routing rule, so no record will be returned.
     */
    ...
    /**
     * we insert records onto partition2, and the 'com.alibaba.cobar.client.entities.Follower.finaByName' action will be performed against partition2
     * too, so each record will be returned as expected.
     */
    ...
    /**
     * WARNING: don't do stupid things such like below, we do this because we can guarantee the shard id will NOT change. if you want to use cobar
     * client corretly, make sure you are partitioning you databases with shard id that will not be changed once it's created!!! <br> with data
     * fixtures setting up on default data source, and update with ShardSqlSessionTemplate.
     */
     ...
     * WARNING: don't do stupid things such like below, we do this because we can guarantee the shard id will NOT change. if you want to use cobar
     * client corretly, make sure you are partitioning you databases with shard id that will not be changed once it's created!!! <br> with data
     * fixtures setting up on another data source, and update with ShardSqlSessionTemplate.
     */
