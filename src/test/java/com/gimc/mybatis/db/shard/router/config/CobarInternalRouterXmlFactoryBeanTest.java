...
import com.gimc.mybatis.db.shard.router.rules.mybatis.AbstractIBatisOrientedRule;
...
public class CobarInternalRouterXmlFactoryBeanTest {
    ...
    ShardInteralRouterXmlFactoryBean factory;

    @BeforeMethod
    protected void setUp() throws Exception {
        factory = new ShardInteralRouterXmlFactoryBean();
    }
    ...
    public void testAssemblingRulesNormally() throws Exception {
        factory.setConfigLocation(new ClassPathResource("com/gimc/mybatis/db/shard/router/config/normal_rule_fixture.xml"));
        factory.afterPropertiesSet();
        ShardInternalRouter router = (ShardInternalRouter) factory.getObject();
        List<Set<IRoutingRule<MyBatisRoutingFact, List<String>>>> rules = router.getRuleSequences();
        assertTrue(CollectionUtils.isNotEmpty(rules));
        assertEquals(4, rules.size());
    }

    public void testAssemblingRulesWithoutConfiguringShards() {
        factory.setConfigLocation(new ClassPathResource("com/gimc/mybatis/db/shard/router/config/abnormal_rule_fixture1.xml"));
        try {
            factory.afterPropertiesSet();
            fail();
        } catch (Exception e) {
    ...
    public void testAssemblingRulesWithoutConfiguringNamspaceOrSqlmap() {
        factory.setConfigLocation(new ClassPathResource("com/gimc/mybatis/db/shard/router/config/abnormal_rule_fixture2.xml"));
        try {
            factory.afterPropertiesSet();
            fail();
        } catch (Exception e) {
    ...
    public void testAssemblingRulesWithConfiguringBothNamspaceAndSqlmap() {
        factory.setConfigLocation(new ClassPathResource("com/gimc/mybatis/db/shard/router/config/abnormal_rule_fixture3.xml"));
        try {
            factory.afterPropertiesSet();
            fail();
        } catch (Exception e) {
    ...
    public void testAssemblingRulesWithInvalidFormatConfiguration() {
        factory.setConfigLocation(new ClassPathResource("com/gimc/mybatis/db/shard/router/config/abnormal_rule_fixture4.xml"));
        try {
            factory.afterPropertiesSet();
            fail();
        } catch (Exception e) {
    ...
    /**
     * load two set of rules with same rule entries, same rule entry should be merge to one.
     */
    public void testAssemblingRulesWithMultipleSameConfigurations() throws Exception {
        ...
        factory.setConfigLocations(new Resource[]{new ClassPathResource("com/gimc/mybatis/db/shard/router/config/normal_rule_fixture.xml"),
            new ClassPathResource("com/gimc/mybatis/db/shard/router/config/normal_rule_fixture2.xml")});
        factory.afterPropertiesSet();
        ShardInternalRouter router = (ShardInternalRouter) factory.getObject();
        List<Set<IRoutingRule<MyBatisRoutingFact, List<String>>>> rules = router.getRuleSequences();
        assertTrue(CollectionUtils.isNotEmpty(rules));
        ...
        for (Set<IRoutingRule<MyBatisRoutingFact, List<String>>> set : rules) {
            assertEquals(1, set.size());
            IRoutingRule<MyBatisRoutingFact, List<String>> r = set.iterator().next();
            if (r instanceof MyBatisNamespaceShardingRule || r instanceof MyBatisSqlActionShardingRule) {
    ...
    public void testAssemblingRulesWithMultiplePartialSameConfiguration() throws Exception {
        factory.setConfigLocations(new Resource[]{new ClassPathResource("com/gimc/mybatis/db/shard/router/config/normal_rule_fixture.xml"),
            new ClassPathResource("com/gimc/mybatis/db/shard/router/config/normal_rule_fixture3.xml")});
        factory.afterPropertiesSet();
        ShardInternalRouter router = (ShardInternalRouter) factory.getObject();
        List<Set<IRoutingRule<MyBatisRoutingFact, List<String>>>> rules = router.getRuleSequences();
        assertTrue(CollectionUtils.isNotEmpty(rules));
