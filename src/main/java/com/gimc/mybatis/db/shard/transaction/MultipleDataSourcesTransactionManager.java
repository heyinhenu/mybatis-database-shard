...
/**
 * use {@link org.springframework.jdbc.datasource.LazyConnectionDataSourceProxy} to wrap all of the data sources we may use in TransactionManager and
 * DAOs. {@link org.springframework.jdbc.datasource.LazyConnectionDataSourceProxy} will only fetch a connection when first statement get executed. So
 * even we start transaction on such data sources which are wrapped by {@link org.springframework.jdbc.datasource.LazyConnectionDataSourceProxy},
...
public class MultipleDataSourcesTransactionManager extends AbstractPlatformTransactionManager implements InitializingBean {
    ...
    /**
     * We need to disable transaction synchronization so that the shared transaction synchronization state will not collide with each other. BUT, for
     * LOB creators to use, we have to pay attention here: <ul> <li>if the LOB creator use standard preparedStatement methods, this transaction
     * synchronization setting is OK;</li> <li>if the LOB creator don't use standard PS methods, you have to find other way to make sure the resources
     * your LOB creator used should be cleaned up after the transaction.</li> </ul>
     */
    @Override
