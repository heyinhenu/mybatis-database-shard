 ...
 * under the License.
 */
package com.gimc.mybatis.db.shard.support.vo;
...
public class ShardMRBase {
    ...
    public ShardMRBase(String[] keys) {
        this(Arrays.asList(keys));
    }

    public ShardMRBase(List<String> keys) {
    ...
    public ShardMRBase(Set<String> keys) {
        if (CollectionUtils.isEmpty(keys)) {
            throw new IllegalArgumentException("empty collection is invalid for hive to spawn data holders.");
        }
        this.keys.addAll(keys);
